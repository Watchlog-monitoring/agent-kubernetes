...
function loadConfig() {
  // اولویت: فایل config/integration.json (برای agent-docker)
  let p = path.resolve(__dirname, "../../config/integration.json");
  // اگر وجود نداشت، از root بخوان (برای watchlog-agent)
  if (!fs.existsSync(p)) {
    p = path.resolve(__dirname, "../../../integration.json");
  }
